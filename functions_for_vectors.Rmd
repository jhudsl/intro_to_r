...
When using `setdiff()` from `dplyr`, it does something different. It tells us the rows in the first tibble that are not in the second tibble. We can select the `State` to compare and it still works, even though `A_states` and `cold_states` are tibbles and not vectors.

```{r}
A_states <- data_As %>% select(State)
cold_states <- data_cold %>% select(State)
dplyr::setdiff(A_states, cold_states)
base::setdiff(A_states, cold_states) # the output is not correct 
```
