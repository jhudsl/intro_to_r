...
## Data Visualization
I decided to make a plot of the top 5 countrie using geom_point() and ggplot().
First I created a new dataset of just the top 10 ranking countries.

```{r}
top20 <- Happiness %>% filter(rank<5)

ggplot(data = top20, aes( x= Country_name, y = Happiness_score )) +
  geom_point()
```
...
ggplot(data = int_Country, aes( x= Country_name, y = Happiness_score ), group = Country_name) +
  geom_boxplot()
```

I can see from this plot that the United States ranked higher than China, India, or Mexico. Mexico ranked second, China third, and India fourth.

# Data Analysis

I then decided to perform a simple analysis to compare countries in different regions. Using `unique()` I determined that there were 10 different regions. I decided to compare countries in `Western Europe` with those of `East Asia`.
...
t.test(x = West_Eur$Happiness_score, y = NAmer$Happiness_score, var.equal = FALSE)
```
