...
**Day/Time:** `r config::get("course_dates")`: 1:30PM-4:50PM ET on Zoom (**`r config::get("holiday")`**.)
...
For example, if you scored 9/10 on an assignment (90%) but turned it in two days late, your score would be 8.6/10 (86%). Note that we must submit final grades by `r config::get("grades_due")`, so please turn in your assignments before this date, so that we can give you credit. 
