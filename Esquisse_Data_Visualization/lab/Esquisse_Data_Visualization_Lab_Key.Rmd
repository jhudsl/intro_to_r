...
Now try creating a plot using the `Orange` data that automatically comes with R using the `esquisse` package. Drag and drop the `age` variable to be plotted on the x-axis. Drag and drop the `circumfrance` variable to be plotted on the y-axis. What happens when you drag `tree` to the facet region of the esquisse GUI? What happens if you move this to color? Copy and paste the code from both options into the chunk below:
