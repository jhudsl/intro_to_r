...
7. Make a new tibble of `BloodType` called `Bloodtype_split` thatsplits the `type` variable into two called `blood_type` and  `Rhfactor`.(Note periods are special characters that generally are interpreted as wild cards thus we need `\\` to tell R that we want it to be interpreted as a period.)
