...
6. Make boxplots using showing `cityTax` (`y` -variable) by whether the property	is a principal residence (`x`) or not.
```{r}
...
10. Make an object called health.sal using the salaries data set, with only agencies of those with `"fire"` (or any forms), if any, in the name remember `fixed( ignore_case = TRUE)` will ignore cases
```{r}
...
12. What is/are the profession(s) of people who have `"abra"` in their name for Baltimore's Salaries?  Case should be ignored
```{r}
...
15. Create a smaller dataset that only includes the Police Department,  Fire Department and Sheriff's Office.  Use the Agency variable with string matching. Call this `emer`.  How many employees are in this new dataset?
```{r}
...
17 (Bonus): convert the 'LotSize' variable to a numeric square feet variable in the tax data set. Some tips: 
