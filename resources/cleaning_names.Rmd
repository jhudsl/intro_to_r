...
The `janitor` package can be really helpful for this!

First let's load the packages we will need. We will show some functions from   `janitor` and we need the `tidyverse`:
...
Nice! That simplified our names very easily! We are using the `str_remove` function on all of our columns to remove the pattern `_percent` where it is found.

## What if we have really complicated names with numbers and punctuation?


We can use patterns with regex - see this [regex cheatsheet](https://evoldyn.gitlab.io/evomics-2018/ref-sheets/R_strings.pdf) for help to remove unwanted characters! We adpated some code from this [source](https://stackoverflow.com/questions/71151470/remove-characters-from-column-names).

First we make some very messy data:
...
- that we want to remove possibly multiple digits with the `+`

- that we want to remove a period (which needs two `\\` based on the cheatsheet too!) and a space
...
## What if we have a row that has names we might want for columns?
...
The function `row_to_names` can be really helpful for this. 
