...
http://jhudatascience.org/intro_to_r/data/tb_incidence.xlsx
...
```{r}
tb %>%
  mutate(before_2000_avg = rowMeans(avgs, na.rm = TRUE)) %>%
  select(country, before_2000_avg)
```

## Summarize the data: `dplyr` `summarize` function

`dplyr::summarize` will allow you to summarize data. If you would like to summarize **all** columns, you can use `across` and pass in a function (with arguments):
