...
A\. Knit your document into a report.

You use the knit button to do this. Make sure all your code is working first!

B\.Using cars, create a new binary (TRUEs and FALSEs) column to indicate if the car has an automatic transmission. Call it "is_automatic".

```{r}
cars <- cars %>% mutate(is_automatic = (Transmission == "AUTO"))
```

C\. What is the average vehicle odometer reading for cars that are both RED and NISSANs? How does this compare with vehicles that do NOT fit this criteria?

```{r}
mean_red_nissan <- cars %>% filter(Color == "RED" & Make == "NISSAN") %>% summarize(mean = mean(VehOdo)) %>% pull()
mean_not_red_nissan <- cars %>% filter(Color != "RED" | Make != "NISSAN") %>% summarize(mean = mean(VehOdo)) %>% pull()
```
...
D\. Among red Nissans, what is the distribution of vehicle ages?
