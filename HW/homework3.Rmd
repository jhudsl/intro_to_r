...
2\. Run the `colnames()` function to take a look at the dataset column names. You should see that there was no name for the first column and that R replaced it with "...1". Rename the first column of "mort" to "country" using the `rename()` function in `dplyr`.  
...
6\. Are there any countries that have a complete record in "mort" across all years? **Hint:** look for complete records by dropping all NAs from the dataset using `drop_na()`.
...
12\. Further subset `long_sub`. You will filter for specific countries using `filter()` and the `%in%` operator. Only include countries in this list: `c("Venezuela", "Bahrain", "Estonia", "Iran", "Thailand", "Canada")`. 
