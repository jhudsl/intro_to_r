...
```{r, fig.alt="Problem gives answer of 1 or 9 depending on calculator.", out.width = "25%", echo = FALSE, out.extra='style="float:left"'}
...
Reassigning allows you to make changes "in place". We assigned `x` already, but we can update it to a new value.
...
## Create vectors with `seq()` {.smaller}
...
-   Can use `:` as shorthand to iterate in increments of 1.
...
You can use the `sample()` function to make a random sequence. The `x` argument specifies what you are sampling from. The `size` argument specifies how many values there should be. The `replace` argument specifies if values should be replaced or not. The default is `replace = FALSE`.
...
```{r, fig.align='center', out.width="50%", echo = FALSE, fig.alt= "Sampling with and without replacement"}
...
-   Some functions have default values for some arguments, so you don't always need to specify all arguments
