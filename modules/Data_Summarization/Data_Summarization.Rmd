...
Use the [`across`](https://dplyr.tidyverse.org/reference/across.html) function with `summarize()` to make iterative summaries across multiple columns of your data.
...
tb %>% 
  summarize(
    across( c(`1990`, `1991`, `1992`, `1993`), ~ sum(.x, na.rm = TRUE))
  )
```
...
Using select helper instead of `c()`:

```{r}
tb %>% 
  summarize(
    across( starts_with("2"), ~ sum(.x, na.rm = TRUE))
  )
```
