...
6\. How many bike lanes are there in each type of lane? Use `table()` on the column named `type`. How does this result differ from #5 ?

```{r}
bike %>%
  pull(type) %>%
  table()
# NAs are NOT included as a category with `table()` !
```
