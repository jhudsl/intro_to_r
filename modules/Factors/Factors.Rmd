...
dropouts_fct %>%
  ggplot(mapping = aes(x = fct_reorder(grade, n_dropouts, .desc = TRUE, mean), y = n_dropouts)) +
  geom_boxplot() +
  labs(x = "Student Grade") +
  theme_bw(base_size = 16)
```

## forcats for ordering.. can be used to sort datasets {.smaller}
...
dropouts_fct %>% pull(grade) %>% levels() # by increasing max dropouts
```
