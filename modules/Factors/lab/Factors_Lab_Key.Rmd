...
0. Load the Youth Tobacco Survey data (using the `jhur` library function `read_yts()`). `select` "Sample_Size",  "Education", and "LocationAbbr". Name this data "yts". 
...
**Bonus / Extra practice**: Convert "LocationAbbr" (state) in "yts_fct" into a factor using the `mutate` and `factor` functions. Do not add a `levels =` argument.

```{r P1response}
yts_fct <- yts_fct %>% mutate(LocationAbbr = factor(LocationAbbr))
```

**Bonus / Extra practice**: We want to create a new column that contains the group-level median sample size. 
...
**Bonus / Extra practice**: We want to plot the "LocationAbbr" (state) by the "med_sample_size" column we created above. Using the `forcats` package, create a plot that:
