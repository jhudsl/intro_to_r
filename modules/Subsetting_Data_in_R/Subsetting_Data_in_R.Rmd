...
{data you are creating or changing} <- rownames_to_column({data you are using}, 
...
When you can, avoid spaces, special punctuation, or numbers in column names, as these require quotes to refer to them. See https://jhudatascience.org/intro_to_r/quotes_vs_backticks.html for more guidance.
...
Note that if you want the values (not a `tibble`), use `pull`:
```{r}
pull(df, mpg)
...
## Odering the colum names of a data frame: alphabetically
