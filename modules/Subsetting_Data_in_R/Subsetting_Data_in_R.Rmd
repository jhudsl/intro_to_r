...
Note that if you want a single vector (not a `tibble`), use `pull` (or `$` if you were using base R):
```{r}
pull(df, mpg)
...
Instead of removing rows like filter, `which()` simply shows where the values occur if they pass a specific condition. We will see that this can be helpful later in more complicated situations.
