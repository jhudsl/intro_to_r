...
## Select multiple adjacent columns with `:`

We can use `select` and a colon `:` to select a set of column names that are next to each other.

```{r}
# year, count, and DOSAGE_UNIT are next to each other
select(AD, year:DOSAGE_UNIT)
```
...
## Select columns of a data frame: `dplyr` {.codesmall}
...
## Nuances about `filter()` {.codesmall}
