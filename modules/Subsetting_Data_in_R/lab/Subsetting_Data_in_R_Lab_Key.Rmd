...
1. Check to see if you have the `mtcars` dataset by running this chunk.
```{r}
mtcars
```
...
4. Copy mtcars into an object called `cars_mt_copy` using the code supplied on line 44. Next, rename `mpg` in `cars_mt_copy` to `MPG` (hint - use `rename()`). 

```{r}
cars_mt_copy <- mtcars
cars_mt_copy <- rename(.data = cars_mt_copy, MPG = mpg)
head(cars_mt_copy)
```

5. Convert the column names of `cars_mt_copy` to all upper case. Use `rename_with()`, and the `toupper` command.
...
```{r}
cars_mt_copy <- rownames_to_column(.data = cars_mt_copy, var = "CAR")
head(cars_mt_copy)
...
```{r}
pvars <- select(.data = cars_mt_copy, CAR, ends_with("P"))
pvars
```
...
```{r}
carsSub <- select(.data = cars_mt_copy, CAR, WT, QSEC, HP)
carsSub
```
...
```{r}
carsSub <- rename_with(.tbl = carsSub, .funs = tolower)
carsSub
```
