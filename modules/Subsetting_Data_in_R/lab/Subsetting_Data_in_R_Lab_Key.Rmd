...
5. Using `cars_mt_copy` create a column called `CAR` using the `rownames_to_column()` function. Create a tibble from the data called `cars_mt_copy`.  Take a look at your data to check it. How many dimensions does it have?
...
**Bonus / Extra practice**: How can you print the first 3 rows and the last 3 rows of `cars_mt_copy` (in two lines of code)?
...
1. Create a subset of the `cars_mt_copy` that only contains the columns: `CAR`, `WT`, `QSEC`, and `HP` and assign this object to `carsSub` - what are the dimensions of this dataset?
...
3. Pull the variable `car` from `carsSub`. How does this differ form selecting it?
...
4. Subset the rows of `cars_mt_copy` that get **more** than 20 miles per gallon (`MPG`) of fuel efficiency - how many are there? Use `filter()`.
...
7. Why do the answers for 2 and 3 differ?
...
carsSub
arrange(carsSub, desc(WT)) %>% head(n = 1) %>% pull(CAR)
```
