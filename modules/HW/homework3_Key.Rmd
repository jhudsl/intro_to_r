...
14\. Use `right_join()` to merge `country_ordered` and `mort_sub` by "country". Then, use the pipe `%>%` to join this merged dataset to `pop`, keeping only the data on the left-hand side of the join. *Hint: use `left_join()`*. Call this final merged dataset `joined`.
