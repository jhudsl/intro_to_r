...
Correlation coefficient is a summary statistic that measures the strength of a linear relationship between two variables.
...
See this [case study](https://www.opencasestudies.org/ocs-bp-co2-emissions) for more information.
...
- by default, Pearson correlation coefficient is computed
- alternative values:
   - two.sided means true correlation coefficient is not equal to zero
...
pull(cor_result, estimate) %>% round(digits = 4)
circ %>%
  ggplot(aes(x = orangeAverage, y = purpleAverage)) +
  geom_point(size = 0.3) + geom_smooth() + annotate("text", x = 2000, y = 7500, label = paste("R =", round(pull(cor_result, estimate), digits =4)))
```
...
circ %>%
  ggplot(aes(x = orangeAverage, y = purpleAverage)) +
  geom_point(size = 0.3) + geom_smooth() + stat_cor(p.accuracy = 0.001)
```
...
Check out this this [case study](https://www.opencasestudies.org/ocs-bp-rural-and-urban-obesity/) and this [case study](https://www.opencasestudies.org/ocs-bp-diet/) for more information.
...
You may also see `lm()` which is a more limited function, only allows for randomly/Gaussian distributed error terms (aka typical linear regressions). 
...
- `glm()` fits regression models:
     ...
     - Use `family = binomial()` to do a other regressions like logistic, Poisson and more
...
- [opencasesstudies.org](www.opencasestudies.org)
