...
## Running two-sample t-test {.smaller}
...
## T-test: retrieving information from the result with `broom` package {.codesmall}
...
* $y_i$ is the outcome for person i
* $\alpha$ is the intercept
* $\beta_1$, $\beta_2$, $\beta_2$ are the slopes/coefficients for variables $x_{i1}$, $x_{i2}$, $x_{i3}$ - average difference in y for a unit change (or each value) in x while the other variables are held constant
...
ice_cream <-
  ...
  mutate(
    ill = case_when(ill == "Y" ~ 1,
                    ill == "N" ~ 0),
    vanilla.ice.cream = case_when(vanilla.ice.cream == "Y" ~ 1,
                                  vanilla.ice.cream == "N" ~ 0)
  )
```
