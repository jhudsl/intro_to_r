...
cor_label = paste("R =", corr_value)
circ %>%
  ggplot(aes(x = orangeAverage, y = purpleAverage)) +
  geom_point(size = 0.3) + geom_smooth() + annotate("text", x = 2000, y = 7500, label = cor_label)
```
...
We fit linear regression model with the number of patient visits (`visits`) as an outcome and total number of hours worked (`hours`) as a predictor. In other words, we are evaluation if the number of hours worked is predicitive of the number of visits a doctor had.
...
```{r}
circ <- circ %>% mutate(day = factor(day,levels = 
...
- `tidy` from the broom package helps us to see results nicely
