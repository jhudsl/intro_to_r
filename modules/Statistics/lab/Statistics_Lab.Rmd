...
Note that it has lots of `NA` values - don't worry if you see that.
...
2. Compute the correlation between the `2006` and `2007` mortality variables with `cor`. (No need to save this in an object. Just display the result to the screen.) Use the `pull()` function to first extract these columns. To use a column name in `pull()` that starts with a number, surround it with backticks. Then, use the `cor` function.
