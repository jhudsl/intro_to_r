...
-   `recode()` can help with simple recoding (not based on condition but
    simple swap)
-   `case_when()` can recode based on conditions
    -   remember `case_when()` needs `TRUE ~ varaible` to keep values
        that aren't specified by conditions, otherwise will be `NA`
-   `stringr` package has great functions for looking for specific parts
    of values especially `filter()` and `str_detect()` combined
    -   also has other useful string manipulation functions like
        `str_replace()` or `str_extract()`
