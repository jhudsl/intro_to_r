...
- `count(variable)`: how manu of each unique value do you have
...
This is also true for logicals.
The NA data could be `> 2` or not, we don't know,so `R` says there is no `TRUE` or `FALSE`, so that is missing.
```{r}
x = c(0, NA, 2, 3, 4, -0.5, 0.2)
x > 2
```
...
Because R can't tell for sure if an NA value meets the condition.
...
## to remove rows with NAs for a variable use `drop_na()`
...
The `na_if()` function of `dplyr` can be helpful for this.
...
- `drop_na()` can help you remove `NA` values - avoid filtering specifically for `NA` values
...
{data_input} %>%
  mutate({variable_to_fix} = case_when({Variable_fixing}   
             condition (==x, >2 ,etc.) ~ {value_for_con})
...
- `stringr` package has great functions for looking for specific parts of values especially `filter()` and `str_detect()` combined
    - also has other useful string manipulation functions like `str_replace()` or `str_extract()`
