...
df %>% select(which(colMeans(is.na(df)) < 0.2))# remove over 20% missing
```
...
Here we use the original values of `Gender` to replace all values of
`Gender` that do not meet the condition `== "M"`.
