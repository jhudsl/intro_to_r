...
```{R}
pct_complete(airqual)
```

Or for a particular variable:

```{R}
airqual %>% select(Ozone) %>%
pct_complete()
```
...
```{r}
airqual %>% filter(Ozone < 5 | is.na(Ozone))
```
...
airqual %>% select(!miss_var_which(airqual))
...
```{r}
df <- tibble(A = c(1, 2, 4, 0), B = c(1, 2, 3, 4))
df %>% mutate(A = na_if(A, 0))
```
...
```{r}
df <- tibble(A = c(1, 2, 4, NA), B = c(1, 2, 3, 4))
df %>% mutate(A = replace_na(A, 0))
```
