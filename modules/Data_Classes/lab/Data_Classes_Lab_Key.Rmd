...
2. Next, create a random vector of 30 values from a set of integars from 1 to 30 called `rand_vect` (hint use `sample()` and set the repeat argument to `TRUE`).
...
```{r}
set.seed(1234)
rand_vect <- sample(x = seq(from = 1, to = 30, by = 1), 
...
5. Take a look at 5 random rows using the `slice_sample()` function again but this time with `set.seed(1234)`before each time`. Try this a few times to see the results. (Don't forget to not copy the backticks.)
