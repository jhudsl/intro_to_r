...
`Double` is equivalent to `numeric`. It is a number that contains <span style="color: red;">fractional values</span>.
...
## Class coersion in with a dataset
...
Say we want to change `daily` to be an integar. We would need to use mutate to help us modify that column or create a new column. Let's create a new one so it is easier to see what is happening. 
...
```{r, error = TRUE}
date("06/15/2021") # This doesn't work
```
...
Note for  function `ymd_hms`: **y**ear **m**onth **d**ay **h**our **m**inute **s**econd. 
