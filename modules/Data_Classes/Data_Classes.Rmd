...
`Integer` is a special subset of `numeric` that contains only <span style="color: red;">whole numbers</span>. Need to use `as.integer` to create integers (unless they are read in as integers or created as such with `seq` and `sample`). Otherwise, will be double by default.
