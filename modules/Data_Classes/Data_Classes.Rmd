...
`Integer` is a special subset of `numeric` that contains only <span style="color: red;">whole numbers</span>. Need to use `as.integer` to create integers (unless they are read in as integars).
...
| 1 	| integer 	| integer 	| Need to coerce to integer with as.integer() 	|
