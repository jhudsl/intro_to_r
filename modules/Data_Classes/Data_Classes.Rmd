...
Need to use `as.integer()` function to create integers (unless they are read in as integers or created as such with `seq` and `sample`). Otherwise, will be double by default.
...
```{r,}
head(iris, n = 3)
iris_mat <- select(iris, -Species) %>% tibble() %>% head(n = 3)
as.matrix(iris_mat)
