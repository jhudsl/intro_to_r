...
## Data classes/types
    ...
    * Double: any number with fractional values (1.2, 4.0)
...
There are two major numeric subclasses or types
...
`Integer` is a special subset of `numeric` that contains only <span style="color: red;">whole numbers</span>. Need to use `as.integer` to create integers (unless they are read in as integars or created as such with `seq` and `sample`). Otherwise, will be double by default.
...
```{r}
tibble(double_var = y,  int_var = y_int)
glimpse(tibble(double_var = y, int_var = y_int))
```
...
* Lists are used when you need to do something iteratively across lots of data - for example wrangling several similar files at once
