...
A\. Why is using `read.csv()` in Question 1 problematic? Look at `colnames()` using both `read_csv()` from the `readr` package and `read.csv()` from base R.
...
# read.csv() attempts to remove special characters (Like + or /) but this can make the column names hard to read!
```
