...
1. Read in the SARS-CoV-2 Vaccination data from http://jhudatascience.org/intro_to_r/data/USA_covid19_vaccinations.csv. Assign the data the name "vacc". We will be reviewing new concepts here and incorporating some from week 1. Remember to use `read_csv()` from the `readr` package.
...
Tweak the code in the block above to look for columns that start with "Total" and select different states/Entities to compare. 
...
8. Read in the GDP and Personal Income Data from http://jhudatascience.org/intro_to_r/data/gdp_personal_income.csv. Call it "gdp". 
...
10. Join the data so that you only retain data in both "vacc" and "gdp". First, try merging without using the `by` argument - what happens? Next, try merging using `by = c("Entity" = "GeoName")`. Call the output "merged". How many observations are there?
...
Perform a `setdiff` on "vacc" and "gdp" to determine what Entities are missing from the GDP data and which are missing from the vaccine data.
