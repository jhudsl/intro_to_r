...
Create a new number `b_num` that is not contained within the vector `nums`. Use your updated `has_n` function from 1.3 with the default value and add `b_num` as the value for the `n` argument when calling the function. What is the outcome?
...
data %>%
  summarize(across(
    .cols = {vector or tidyselect},
    .fns = function(x) {some function}(x, {additional arguments})
  ))
```
