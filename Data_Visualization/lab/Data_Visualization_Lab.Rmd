---
title: "Data Visualization Lab Key"
output: html_document
editor_options: 
  chunk_output_type: console
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```




# Part 1

0. Load the libraries

```{r, message=FALSE}
library(readr)
library(ggplot2)
library(dplyr)
library(jhur)
```

0. Read Bike Lanes Dataset using `read_bike()` function from `jhur` package. Assign it to `bike` variable. 

Then, use the provided code to compute a data frame `bike_agg` with aggregate summary of bike lanes: average length of lanes (`lane_avg_length`) for each year (`dateInstalled`). 

```{r}
bike = read_bike()

bike_agg <- 
  bike %>%
  # filter data to keep only these observations for which year is non-0
  filter(dateInstalled != 0) %>%
  group_by(dateInstalled) %>%
  summarise(lane_avg_length = mean(length)) 

bike_agg

```

1. Use base R graphic to make plot of average length of lanes (`lane_avg_length`; y-axis) for each year (`dateInstalled`; x-axis).

Try using point plot type (default) and line type  (use argument `type = "l`). 

```{r}

```

2. Use `ggplot2` package make plot of average length of lanes (`lane_avg_length`; y-axis) for each year (`dateInstalled`; x-axis). You can use lines layer (`+ geom_line()`) or points layer (`+ geom_point()`), or both!

Assign the plot to variable `my_plot`. Type `my_plot` in the console to have it displayed 

```{r}
# ggplot(???, aes(x = ???, y = ???)) + 
#   geom_???() + 
#   geom_???()

# my_plot <- ???
```

3. "Update" your plot by adding additional elements, e.g. plot labels. 

```{r}
# my_plot <- 
#   my_plot + 
#   labs(???)
# 
# my_plot
```

4. You can use `scale_x_continuous()` function and its arguments to modify x-axis look. You can use `scale_y_continuous()` function and its arguments to modify y-axis look. Test this code out!

```{r}
# my_plot <- 
#   my_plot + 
#   scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013)) + 
#   scale_y_continuous(limits = c(0, NA))  # force y-axis to start from 0, do not force upper bound 
#   
# my_plot
```


5. Observe different versions of the plot by displaying `my_plot` while adding a different "theme" to it. Test this code out!

```{r}
# my_plot + theme_bw()
# my_plot + theme_classic()
# my_plot + theme_dark()
# my_plot + theme_gray()
# my_plot + theme_void()
```


# Part 2

Use the provided code to compute a data frame `bike_agg_2` with aggregate summary of bike lanes: number of lanes (`lane_count`) -- separately for each year (`dateInstalled`) and for each lane type. 

```{r}
bike_agg_2 <- 
  bike %>%
  filter(dateInstalled != 0) %>%
  group_by(dateInstalled, type) %>%
  summarise(lane_count = n())

bike_agg_2
```


1.  Use `ggplot2` package to make a plot showing trajectories of number of lanes (`lane_count`; y-axis) over year (`dateInstalled`; x-axis), where each bike line type has a different line (hint: use `group = type` in mapping) and a different color (hint: use `color = type` in mapping). 

```{r}
# ggplot(???, aes(x = ???, y = ???, group = ???, color = ???)) + 
#   ???
```

2. Redo the above plot by adding a faceting  (`+ facet_wrap(~ type, ncol = 3)`) to have data for each bike line type in a separate plot panel. 

(You may see `geom_path: Each group consists of only one observation. Do you need to adjust the group aesthetic?` warning as some bike lane types will have only 1 point plotted while trying to plot a line).

```{r}
# ggplot(???, aes(x = ???, y = ???, group = ???, color = ???)) + 
#   ???
```
