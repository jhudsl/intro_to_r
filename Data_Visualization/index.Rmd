...
- A package for producing graphic 
...
* **Aesthetic mapping** (here: `aes(x = var_1, y = var_2)`) that describes how variables in our data are mapped to elements of the plot 
...
## Group and color by variable's values 
...
We can experiment with different layer: use boxplot instead of lines. 

Again: we `group` element in a mapping to indicate that each `item_ID` will have a separate price boxplot (more generally: a separate layer element)
...
We use `fill` element in a mapping to indicate that each `item_categ` will have different color of boxplot filling used.
...
## Split plot into panels by variable
