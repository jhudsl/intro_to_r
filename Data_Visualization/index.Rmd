...
ggplot(food, aes(x = observation_time, 
                 y = item_price_change,
                color = item_ID)) + 
  geom_line()  +facet_grid(~item_categ)
```
## Adding a facet can help make it easier to see what is happening

Two options:
`facet_grid()`- creates a grid shape
`facet_wrap()`- more flexible - arguments `ncol` and `nrow` can specify layout
...
`geom_bar()` can only one `aes` mapping
`geom_col()` can have two
```{r, fig.width=6 , fig.height=4 * 0.8, eval = FALSE}
ggplot(food, aes(x = item_ID, 
                 y = item_price_change, 
                 fill = item_categ)) + 
...
<!--                  group = item_categ, -->
<!--                  fill = item_categ)) +  -->
<!--   geom_boxplot() +labs(x = "Item ID", y = "Item prices", fill = "Item\ncategory") -->
<!-- ``` -->

<!-- ## Group and/or color by variable's values -->

<!-- We use `fill` element in mappings to indicate that each `item_ID` will have different **color of boxplot filling** used. -->
...
<!--                  group = item_ID, -->
<!--                  fill = item_ID)) +  -->
<!--   geom_boxplot() +labs(x = "Time", y = "Item prices", fill = "Item\ncategory") -->
<!-- ``` -->
