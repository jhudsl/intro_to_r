...
2. Create new variables/make rownames a column
3. Rename columns of a data frame
4. Subset rows of a data frame
5. Subset columns of a data frame
6. Add/remove new columns to a data frame
7. Order the columns of a data frame
8. Order the rows of a data frame
...
To do this we will use the `tibble` package which is also part of the `tidyverse`.
...
If we would like to create a `tibble` ("fancy" data frame), we can using the  `as_tibble()` function (from the `tibble` package - part of the `tidyverse` too!).  
...
## Making rownames into a new column

To do this we need the `mutate()` function of the `dplyr` package. This is a super helpful function!

Can **modify** or **create** new variables.
