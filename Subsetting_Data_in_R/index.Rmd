...
The `dplyr` package also interfaces well with tibbles (a fancy tidyverse version of a data frame).
...
In the "tidy" data format, all information of interest is a variable (not a name). **as of tibble 2.0, rownames are removed**.  For example, `mtcars`  has each car name as a row name. Here we use the `head()` function to see the first 2 rows of each.
...
For example, let's rename mpg to MPG.
...
To rename all columns you use the `rename_all` command (with a function). In this case we will use `toupper()` to make all letters upper case. Could also use `tolower()` function.
...
[Website](http://https://jhudatascience.org/intro_to_r/index.html)
...
The `one_of()` function checks for columns with the names indicated. If there are columns that match they will get removed. This is useful if you can't recall if the columns you want to remove are still present.

Otherwise you could use `c()` to list the columns to remove.
