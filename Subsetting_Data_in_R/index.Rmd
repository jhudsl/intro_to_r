...
If we would like to create a `tibble` ("fancy" data.frame), we can using `as.tbl`.  In the "tidy" data format, all information of interest is a variable (not a name). **as of tibble 2.0, rownames are removed**.
...
Combined with `ifelse(condition, TRUE, FALSE)`, it can give you didffer
```{r}
df = mutate(df, 
            disp_cat = ifelse(
              disp <= 200,
