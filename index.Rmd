...
* 8:30AM-11:50PM in W4013 on January 8-12
...
#### Packages 
All the packages used (in any way - homework, lecture, lab), are listed in [this text file](all_the_packages.txt).  If you would like to install all the packages from this file, please download that text file and run [this R script](install_all_packages.R)
...
df = matrix(
  ...
  "Fixing problems in read_csv with col_types", "read_csv_specifications.R")
, ncol = 2, byrow = TRUE)
...
In `R`, if you have never set an `Rprofile` before, you can make `tidyverse` load whenever you start `R` as follows:
...
if (any(have_r_pack)) {
  stop("add R_DEFAULT_PACKAGES to .Rprofile manually!")
} else {
