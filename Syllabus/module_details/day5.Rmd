...
1. Identify factors in your data.
2. Develop strategies for addressing common raw data quality issues and note potential data wrangling pitfalls.
3. Expand your data cleaning toolkit by exploring the tidyverse, a curated suite of packages for data science. 
  ...
  schedule  %>%
    ...
    filter(
      row_number() >= which(str_detect(schedule$`**Day**`, "\\[Day 5\\]")) &
        row_number() < which(str_detect(schedule$`**Day**`, "\\[Day 6\\]")) - 1
    ),
  missing = "",
  split.tables = Inf,
  style = "rmarkdown"
