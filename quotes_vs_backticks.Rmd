...
library(kableExtra)
text_tbl <- data.frame(
    Function = c("tibble()", "rename()", "filter()","summarize()", "mutate()", "stringr functions", "pivot_longer()/pivot_wider()"),
    Use = c("Tolerates quotes or backticks if atypical", "Tolerates quotes or backticks if atypical", "Only backticks if atypical", "Only backticks if atypical","Tolerates quotes or backticks if atypical", "Only quotes as it works with character strings", "Only quotes tollerated in all cases")
)
kbl(text_tbl) %>%
  kable_paper(full_width = F, html_font = "helvetica", font_size = "18") %>%
