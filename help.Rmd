...
If you never created trees2 and try to modify it like so:
```{r, eval = FALSE}
trees2 +1
```
...
- You may get a `+`  sign instead of the cursor `>` in the console suggesting that you have not closed an open parnetheses. Each instance of `(` should be followed by `)`. This can also result in different errors like so:
...
```{r, eval = FALSE}
libary(tidyverse) # need this! This can be at the top of your file
