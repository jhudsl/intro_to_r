...
## **`Error in `validate_mapping()` `mapping` must be created by `aes()`:: Forgetting to use `+` to add `ggplot2` layers**
...
## **`Error in `+.gg`! Cannot use `+.gg()` with a single argument. : Using `+` at the start of a line  not the end of the previous line to add `ggplot2` layers**
